...
const char* message = "follower:checkin";
...
{ // key at AVW 1338, take video of it running in case it doesn't work day of 17th
    ...
    while (!checkin_complete)
    {
        ...
        if (client.connect(host, port))
        {
            ...
            {
                message = "follower:active";
                checkin_complete = true;
            }
            else if (recd_message == "reset")
            {
                message = "follower:checkin";
                return;
            }
        }
        else
    ...
    while(!maze_finished) {
      ...
      bool recieved_instruction = false;
      std::string command = "none";
      Serial.begin(9600);

      while (!recieved_instruction) {
        WiFiClient client;
        if (client.connect(host, port)) {
          client.println(message);
          String response = client.readStringUntil('|');
          ...
          if (recd_message == "maze_complete") {
            message = "follower:complete";
            maze_finished = true;
            return;
          } else if (recd_message == "right_" + std::to_string(counter)) {
            command = "right";
            recieved_instruction = true;
          } else if (recd_message == "left_" + std::to_string(counter)) {
            command = "left";
            recieved_instruction = true;
          } else if (recd_message == "forward_" + std::to_string(counter)) {
            command = "forward";
            recieved_instruction = true;
          } else if (recd_message == "backward_" + std::to_string(counter)) {
            command = "backward";
            recieved_instruction = true;
          }
      ...
      counter++;
      std::string temp = "nav_" + std::to_string(counter);
      message = temp.c_str();
      recieved_instruction = false;
      command = "";
