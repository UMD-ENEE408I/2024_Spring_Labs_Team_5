...
void turn_left90(Encoder& enc1, Encoder& enc2){
  ...
  while(enc2_value > -200){//3 3/8 inches -240 - try sensor fusion
...
void turn_right90(Encoder& enc1, Encoder& enc2){
  ...
  long enc1_value = enc1.read();
  long enc2_value = enc2.read(); //right motor
  while(enc1_value < 230){//3 3/8 inches 240 - try sensor fusion, 280 maybe
...
void right_turn_till_line(Encoder& enc1, Encoder& enc2){
  ...
  long enc2_value = enc2.read(); //right motor
  int time_to_turn = 0;
  while(true){//3 3/8 inches 240 - try sensor fusion, check position of line
    readADC();
    digitalConvert();
    error = enc2_value - enc1_value; 
    total_error += error;
...
void maze_straight(Encoder& enc1, Encoder& enc2){
  ...
  while((dist_travelled < 10) && (move_once == 0)){
      //travel forwards
      readADC();
      digitalConvert();
      float pos = getPosition(previousPosition);
...
void loop() {//key at AVW 1338, take video of it running in case it doesn't work day of 17th
  ...
  /* //start of code not needed for Abheek's robot
  right_turn_till_line(enc1,enc2);
