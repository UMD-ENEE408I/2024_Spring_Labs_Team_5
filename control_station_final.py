...
def traverse_maze(counter):
        print("Traverse Maze")
        global currentState
        currentState.status = Rounds.ALLACTIVE
        
        if counter != currentState.maze_counter:
                print("counter != currentState.maze_counter")
                return
        ...
        if len(currentState.movement_queue) == 0:
                ...
                currentState.follower_message = command+"_"+str(currentState.maze_counter)
                print("current state follower message after traverse: " + currentState.follower_message)

        currentState.maze_counter += 1
...
def handleStatus():
        ...
        if currentState.status == Rounds.CHECKINS:
                if (currentState.FollowerCheckIn == True and currentState.LeaderCheckIn == True):
                        currentState.status = Rounds.READY
                        print(" ---------------------->> All Robots Ready- Press Enter to Begin")
                        readyBlock = input()
        elif currentState.status ==  Rounds.READY:
                if verbose: print("Status -> READY")
                currentState.leader_message = "begin"
                currentState.follower_message = "begin"                
        elif currentState.status == Rounds.ALLACTIVE:
                pass
                #print("All Active!")
        elif currentState.status == Rounds.WAITFORMAZE:
                return
        elif currentState.status ==  Rounds.STRAIGHTAWAY:
                return
        elif currentState.status ==  Rounds.COMPLETE:
                return
    ...
    if currentState.status != Rounds.CHECKINS:
        if "leader" in message:
            if "audio" in message:
                ...
                if "nav_" in message:
                        print("EXTRACTED COUNTER #"+str(message[message.find("nav_")+4:message.find("nav_")+6]))
                        traverse_maze(int(message[message.find("nav_")+4:message.find("nav_")+6]))
                ...
                try:
                        while True:
                                ...
                                try:
                                        ...
                                        if len(str(data.decode()).strip()) != 0: print("SENT: "+currentState.message)
